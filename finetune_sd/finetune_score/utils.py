...
def log_to_wandb(images, bad_images, captions, scores, bad_scores, global_step):
    logger.info("Uploading to wandb")
    images = [wandb.Image(img) for img in images]
    bad_images = [wandb.Image(img) for img in bad_images]
    predictions = [captions, images, bad_images, scores, bad_scores,
                   [global_step] * len(captions)]
    columns = ["prompt", "image", "bad_image", "score", "bad_score", "global_step"]
